...
function dynamicPyCode() {
  return {
    name: 'dynamic-pycode',
    transform(code, id) {
      // Target the file where you want to replace __PYCODE__
      console.log(id);
      if (id.endsWith('nodebuilderworker.mts')) {
        const pycode = fs.readFileSync(path.resolve(__dirname, "src", "nodebuilder.py"), 'utf8');
        return {
          code: code.replace('__PYCODE__', JSON.stringify(pycode)),
          map: { mappings: '' }
        };
      }
      return null;
